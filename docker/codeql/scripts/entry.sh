...
# Copyright (c) 2025, Thanh Ba Nguyen
...
do_analyze() {
    ...
    print_blue   "DEBUG: codeql database create --build-mode=none --overwrite --language=${LANGUAGE} -s ${src_dir} ${results_dir}/codeql-db"
    codeql database create --build-mode=none --overwrite --language=${LANGUAGE} -s ${src_dir} ${results_dir}/codeql-db
    if [ $? -eq 0 ]
    then
